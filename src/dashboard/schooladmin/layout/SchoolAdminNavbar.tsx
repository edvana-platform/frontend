...
export function SchoolAdminNavbar({ isMobileOpen, setIsMobileOpen }: SchoolAdminNavbarProps) {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:block">
            <h1 className="text-xl font-semibold text-white">Green Hills Academy | School Admin</h1>
          </div>
          ...
          <div className="relative">
            <DropdownMenu>
              ...
              <DropdownMenuContent 
                align="end" 
                sideOffset={8}
                className="w-64 bg-white/20 backdrop-blur-xl border border-white/30 shadow-2xl rounded-xl"
              >
                <div className="px-4 py-3 border-b border-white/20">
                  <h4 className="text-sm font-medium text-white mb-1">{user.fullName}</h4>
                  <p className="text-xs text-white/80 mb-1">{user.email}</p>
                  <p className="text-xs text-white font-medium">{user.role}</p>
                  <p className="text-xs text-gray-600">{user.school}</p>
                </div>
                <DropdownMenuItem 
                  onClick={handleSettings}
                  className="px-4 py-2 text-sm text-white hover:text-white hover:bg-white/20 cursor-pointer transition-colors rounded-xl mx-2 my-1"
                >
                ...
                <DropdownMenuItem 
                  ...
                  className="px-4 py-2 text-sm text-red-600 hover:text-red-700 hover:bg-red-50/50 cursor-pointer transition-colors rounded-xl mx-2 my-1"
                >
